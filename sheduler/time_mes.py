...
def _format_utc0(dt: datetime) -> str:
    """Naive datetime считается UTC+0 (как в выборке и _in_send_window)."""
...
async def send_message_cron(bot: Bot):
    ...
    if CHECKER_ID is not None:
        await bot.send_message(
            CHECKER_ID,
            f'Начинаю рассылку (окно UTC+0: {_format_utc0(now)} — {_format_utc0(window_end)}, '
            f'длительность {WINDOW}, кандидатов (точная выборка по окнам): {len(candidate_rows)})',
        )
    sent_count_7 = 0
    sent_count_3 = 0
    sent_count_1 = 0
        ...
        try:
            ...
            if now < end:
                ...
                if (
                    ...
                    try:
                        response = await x3.updateClient(3, user_id_str, user_id)
                        if response:
                            ...
                            logger.error(
                                f"❌ Не удалось добавить 3 дня пользователю {user_id} (second_chance)"
                            )
                    except Exception as e:
                        logger.error(f"Ошибка при добавлении 3 дней пользователю {user_id}: {e}")
