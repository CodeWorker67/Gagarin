...
_ADD7ALL_PROMO_TEXT = (
    "Самое время вернуться в Gagarin VPN — дарим 7 дней тестдрайва новых серверов🟢\n\n"
...
_ADD7SUB_TEXT = (
    ...
    "Проблема решена - личный кабинет снова заработал.\n\n"
...
async def check_online(message: Message):
    squad_1 = ['b8031c60-2861-49ab-a8ec-0fb871c7be23']
    squad_2 = ['d963e9fb-044a-4188-9938-e34f2eee9650']
    success_count = 0
    fail_count = 0
    if message.from_user.id not in ADMIN_IDS:
        return
...
async def sync_panel(message: Message):
    ...
    if CHECKER_ID is not None:
        await bot.send_message(
            ...
            'доступа.\nПерейдите в кабинет, нажав «🚀 Подключить Gagarin VPN»',
...
async def send_gift_command(message: Message):
    ...
    # Текст сообщения
    gift_text = '''
☄️ Это была DDoS-атака!

Друзья, простите за временные неудобства. Сервис мог кратковременно работать нестабильно — сейчас всё в порядке.

Мы столкнулись с мощной DDoS-атакой; если у вас <b>не открывался личный кабинет — проблема уже решена.</b>

☄️ Мы начислили вам <b>дополнительные 5 дней</b> к подписке, чтобы вы могли оценить <b>Gagarin VPN</b>.

📱 Не можете настроить?
Если вы никак не могли разобраться с импортом конфигов — <b>смотрите видеоинструкцию</b>! Там всё разложено по полочкам.

🚀 Осталось только нажать «🚀 Подключить Gagarin VPN» — и вы в деле.
            '''
...
🚀 Осталось только нажать «🚀 Подключить Gagarin VPN» — и вы снова в деле.
    '''
...
_ADD_NEW_USERS_CUTOFF = datetime(2026, 6, 6, 0, 0, 0)
...
async def add_7_to_all_command(message: Message):
    """
    Рассылка пользователям без подписки или с подпиской, истёкшей 2+ дня назад.
    ...
    if not user_ids:
        await message.answer(
            "Нет пользователей: is_delete=False, нет подписки "
            "(subscription_end_date пусто) или она истекла 2+ дня назад (UTC)."
        )
        return
    ...
    await message.answer(
        ...
        f"(is_delete=False; нет подписки или истекла ≥2 дней назад по subscription_end_date UTC).\n\n"
