 ...
 *
 * Gagarin VPN: POST на {api}/api/v1/sub_page/pay/{fk_sbp|fk_card|stars|cryptobot}
 * с телом { user_id, duration }. Заголовок X-Sub-Page-Api-Key = SUB_PAGE_API_KEY из .env бота.
 *
 * При сборке Vite (префикс VITE_):
 *   VITE_SUB_PAGE_PAY_API_BASE — по умолчанию http://bot.gagarin.today (если не задан)
 *   VITE_SUB_PAGE_PAY_API_KEY — обязателен (тот же SUB_PAGE_API_KEY, что в .env бота)
 *
 * В /opt/remnawave/.env.sub (или env_file контейнера subscription-page) задайте ключ;
 * при необходимости переопределите базовый URL (https и т.д.).
 *
 * Блок «Оплата»: Mantine Accordion — свёрнут при активной подписке, развёрнут при истёкшей
 * (как свёрнутый блок информации о подписке). Сообщения об ошибке конфигурации всегда в развёрнутой панели.
 */
...
const DEFAULT_SUB_PAGE_PAY_API_BASE = 'http://bot.gagarin.today'
...
function SubscriptionPayBlock({ isMobile }: { isMobile: boolean }) {
    ...
    const [payAccordion, setPayAccordion] = useState<string | null>(null)

    const hasSetupError = !payCfg.apiKey || userId == null
    ...
    const panelBody = !payCfg.apiKey ? (
        <Text c="dimmed" size="sm">
            Оплата: задайте VITE_SUB_PAGE_PAY_API_KEY при сборке фронта (тот же SUB_PAGE_API_KEY, что в .env
            бота). При необходимости задайте VITE_SUB_PAGE_PAY_API_BASE.
        </Text>
    ) : userId == null ? (
        <Text c="dimmed" size="sm">
        ...
        <>
            <Card p={0} radius="lg" withBorder>
                <Accordion
                    ...
                    onChange={setPayAccordion}
                    value={payAccordion}
                    variant="contained"
                >
                    <Accordion.Item value="pay" style={{ border: 'none' }}>
                        <Accordion.Control>
